...
**Ngày phân tích:** 13/11/2025 20:43:32  
...
curl -I https://attech.online | grep -i "x-frame\|content-security"

# Hoặc test từng cái:
curl -I https://attech.online | grep -i "x-frame-options"
...
# 2. Test đầy đủ
curl -v https://attech.online 2>&1 | grep -i "frame"

# 3. Scan lại bằng tool
python3 scanClickjacking.py https://attech.online
```
...
|--------|-------------------|---------|--------|
| X-Frame-Options | `DENY` hoặc `SAMEORIGIN` | `DENY` | ✅ |
| CSP frame-ancestors | `frame-ancestors 'none'` | `frame-ancestors 'none'` | ✅ |
| HSTS | `max-age=31536000` | `❌ Không có` | ⚠️ |

**Quick test command để verify:**

```bash
curl -I https://attech.online | grep -i "x-frame\|content-security\|strict-transport"
```

**Chi tiết từng header:**

✅ **X-Frame-Options:** `DENY`
   - Bảo vệ tối đa: Chặn hoàn toàn việc nhúng vào iframe
...
**Lý do:** X-Frame-Options: DENY chặn hoàn toàn, CSP frame-ancestors 'none' chặn hoàn toàn
...
|----------|---------|----------|------|
| X-Frame-Options | ✅ DENY | Bảo vệ tối đa | 50/50 |
| CSP frame-ancestors | ✅ 'none' | Bảo vệ tối đa | 50/50 |

**Tổng điểm bảo vệ:** 100/100

✅ **Đánh giá:** BẢO VỆ TỐT - An toàn trước clickjacking
...
### ✅ Đã được bảo vệ tốt, nhưng có thể tăng cường thêm
...
<body>
    ...
    <iframe src="https://attech.online" width="800" height="600"></iframe>
...
```http
Server: nginx/1.29.3
Date: Thu, 13 Nov 2025 13:42:59 GMT
Content-Type: text/html
Transfer-Encoding: chunked
Connection: keep-alive
Last-Modified: Thu, 13 Nov 2025 12:34:08 GMT
